            ...
            if (!args[0]) sendAvailableArenaMessage(); else {
                switch (args[0]) {
                    ...
                    case "spawnpoint":
                        ...
                        break;
                    case "close":
                        util.warn(`[${global.gameManager.host}/${global.gameManager.port}]: ${socket.player.body.name === "" ? `An unnamed player (ip: ${socket.ip})` : socket.player.body.name} has closed the arena.`);
